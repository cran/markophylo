...
- Custom substitution rate matrices can be supplied.
...
```
Now, using the function *estimaterates* from the *markophylo* package, we will estimate a substitution rate matrix from this data. The *modelmat* argument represents the number of unique rates to be fit using a 2-dimensional matrix, with the diagonals left as NA (see section on creating *modelmat* below).
```{r}
...
print(model1)
```
The output from printing *model1* (which is an object of class 'markophylo') is self-explanatory. The estimated rates (given as a array in *$rates*) and the standard errors (both indexed according to the parameter index matrix) are output. The log-likelihood at the maximum likelihood estimates along with the values of the model selection criteria (Akaike information criterion and Bayesian information criterion) are also provided. Because we did not specify *a priori* any clades to be fit with their own unique rates, these estimates are homogenous over the tree. This is also reflected in the one component list output under "Groups of nodes with the same rates". Note that the *model1* object is of class *markophylo*.
```{r}
print(class(model1))
```
Among other things, *model1* contains the number of unique patterns observed that can be retreived using `model1$w`. As expected, for binary discrete characters for four taxa, there are `2^4` unique patterns.
...
Any arbitrary square matrix can be supplied to the function *estimaterates* where the zero entries denote substitutions that are not permitted, and the unique non-zero integer entries denote the rates that must be estimated. 
